...
counter.innerHTML = 'FINALLY TOGETHER 💗🏡👫'

/*

setInterval(() => {
  const timeOfTravel = new Date("2026-03-27").getTime() + new Date().getTimezoneOffset() * 60 * 1000;
  ...
  //counter.innerHTML = `TIME SINCE VISA APPOINTMENT 🪪🏢: ${days}:${String(hours).length < 2 ? "0" + hours : hours}:${String(minutes).length < 2 ? "0" + minutes : minutes}:${
